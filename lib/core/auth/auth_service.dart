...
class AuthStateNotifier extends StateNotifier<AsyncValue<User?>> {
  ...
  /// True when the current session was created by a fresh sign-up.
  /// Used by [MainNavigationHub] to force the onboarding flow.
  ...
  Future<void> _init() async {
    _authSub = _client.auth.onAuthStateChange.listen((data) {
      if (data.session != null) {
        ...
        isNewUser = false; // reset on logout so it doesn't bleed into the next session
      }
    ...
    try {
      ...
      // Secure storage reads can hang on first launch; Supabase Auth is the source of truth.
    }
  }
  ...
  Future<void> _handleAuth(Future<AuthResponse> Function() action, {bool markAsNew = false}) async {
    ...
    try {
      // Set the flag before await so it's ready when the auth state listener fires synchronously.
  ...
  /// Called by [MainNavigationHub] after the onboarding screen has been shown once.
