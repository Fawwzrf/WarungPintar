...
class _LoginScreenState extends ConsumerState<LoginScreen> {
  ...
  // [LOW-03 FIX] Dispose controllers to prevent memory leak
  @override
  void dispose() {
    _email.dispose();
    _pass.dispose();
  ...
  Widget build(BuildContext context) {
    ...
    final String modeLabel = _mode == AuthMode.login ? 'MASUK' : _mode == AuthMode.signup ? 'DAFTAR' : 'RESET KATA SANDI';
        ...
        body: Center(
          child: SingleChildScrollView(
            ...
            child: Column(children: [
              Text(modeLabel, style: Theme.of(context).textTheme.headlineMedium),
              const SizedBox(height: 32),
              TextField(controller: _email, decoration: const InputDecoration(labelText: 'Email', border: OutlineInputBorder())),
              const SizedBox(height: 16),
              if (_mode != AuthMode.reset) ...[
                TextField(controller: _pass, decoration: const InputDecoration(labelText: 'Kata Sandi', border: OutlineInputBorder()), obscureText: true),
                const SizedBox(height: 16),
              ],
              if (state.isLoading) const CircularProgressIndicator()
                ...
                child: Text(_mode == AuthMode.reset ? 'Kirim Tautan Reset' : 'Masuk'),
              ),
              TextButton(
                onPressed: () => setState(() => _mode = _mode == AuthMode.login ? AuthMode.signup : AuthMode.login),
                child: Text(_mode == AuthMode.login ? 'Buat Akun' : 'Kembali ke Masuk'),
              ),
