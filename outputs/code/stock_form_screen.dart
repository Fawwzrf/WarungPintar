...
class _StockFormScreenState extends ConsumerState<StockFormScreen> {
  ...
  Future<void> _submit() async {
    ...
    if (parsed == null || parsed <= 0) {
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Enter a valid positive whole number')));
      return;
    }
    ...
    if (!_isAddition && parsed > widget.product.stock) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('Cannot reduce by $parsed — current stock is only ${widget.product.stock}')),
      );
      return;
    }
    ...
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('Adjustment failed: ${e.toString().replaceAll('Exception: ', '')}'), backgroundColor: Colors.red[700]),
        );
      }
    } finally {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: Text('Stock: ${widget.product.name}')),
      body: _loading
        ? const Center(child: CircularProgressIndicator())
        : Padding(
            padding: const EdgeInsets.all(16.0),
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              Text('Current Stock: ${widget.product.stock}', style: Theme.of(context).textTheme.titleMedium),
              const SizedBox(height: 16),
              Row(children: [
                Expanded(child: ChoiceChip(
                  label: const Text('Add Stock'), selected: _isAddition,
                  onSelected: (_) { setState(() { _isAddition = true; if (_reason == 'sale') _reason = 'restock'; }); },
                )),
                const SizedBox(width: 8),
                Expanded(child: ChoiceChip(
                  label: const Text('Reduce Stock'), selected: !_isAddition,
              ...
              TextField(
                controller: _amount,
                decoration: const InputDecoration(labelText: 'Amount', border: OutlineInputBorder(), hintText: 'e.g. 10'),
                keyboardType: TextInputType.number,
                onSubmitted: (_) => _submit(),
              ),
              const SizedBox(height: 16),
              DropdownButtonFormField<String>(
                value: _reason, decoration: const InputDecoration(labelText: 'Reason', border: OutlineInputBorder()),
